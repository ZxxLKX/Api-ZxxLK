...
router.get('/maker/circle', async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] cek kembali url image"}) 
...
router.get('/maker/darkness', async (req, res) => {
	...
	var n = isNumber(no)
	if ( !img ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] cek kembali url image"}) 
	if ( !n ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] parameter no nombor saja"}) 
...
router.get('/maker/resize', async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] cek kembali url image"}) 
	if ( !wid ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] parameter width nombor saja"}) 
	if ( !hei ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] parameter height nombor saja"}) 
...
router.get('/maker/welcome1', async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter member"})  
	if (!pp ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter pp"})  
    if (!bg ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] cek kembali url image bg"}) 
...
router.get('/maker/goodbye1', async (req, res, next) => {
	...
	if ( !imgpp ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter pp Link pp dengan betul"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'ZxxLK', message : "[!] masukan parameter pp Link bg dengan betul"}) 
